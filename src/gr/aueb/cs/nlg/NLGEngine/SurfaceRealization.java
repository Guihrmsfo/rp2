...
{	
    ...
    public String Realize(XmlMsgs MyXmlMsgs, String ut)
    {
        ...
        if (type==2 || type ==1)
        {
            ...
            {                 
                    ...
                    // we found a comparator that must be realized into text
                    // (the first slot of the msg is a Comparator
                    if (Msg!=null && Msg.getFirstChild()!=null && Msg.getFirstChild().getLocalName()!=null 
                            && Msg.getFirstChild().getLocalName().equalsIgnoreCase("Comparator"))
                    {
                            AnnotDescription.startComparator(Msg);
                            
                            // get the slots of the Comparator
                            Node comparison = Msg.getFirstChild();
                            Vector MsgSlots = XmlMsgs.ReturnChildNodes(comparison);

                            // turn Comparator slots to text
                            ...
                            if (!REALIZED_TEXT.substring(REALIZED_TEXT.length()-"και ".length(),REALIZED_TEXT.length()).equalsIgnoreCase("και "))
                            {
                                if (REALIZED_TEXT.charAt(REALIZED_TEXT.length()-1) == ' ')
        ...
        if(this.shape_Text)
            ret = shapeText(REALIZED_TEXT);
        else
            ret = REALIZED_TEXT.toString();
    ...
    private void realize_slots(StringBuffer REALIZED_TEXT, Vector MsgSlots,XmlMsgs MyXmlMsgs)
    {
        try
        {
            ...
            for(int j = 0; j < MsgSlots.size(); j++)// for each slot
            {
                ...
                if(XmlMsgs.compare(Slot, XmlMsgs.prefix, XmlMsgs.VERB_TAG))
                {
                    ...
                    {
                        String ret  = MyXmlMsgs.getAttribute(Slot,XmlMsgs.prefix,XmlMsgs.singular_VERB_TAG);
                        AnnotDescription.add_VERB(ret, get_for_Property(Slot) , get_Interest(Slot), get_Assim(Slot));
                        addText(ret);
                    }
                }
                ...
                else if(XmlMsgs.compare(Slot, XmlMsgs.prefix, XmlMsgs.IS_A_TAG)) // if IS_A
                {//an to slot einai IS_A
                ...
                {                
                        ...
                        REALIZED_TEXT.append(", ");
                        AnnotDescription.add_TEXT(",");
                        //AnnotDescription.add_Whitespace();
                }
		     ...
		     String re_type = XmlMsgs.getAttribute(Slot, XmlMsgs.prefix, XmlMsgs.RETYPE);
                     ...
                     if (owner_URI.equalsIgnoreCase(""))
                         owner_URI = MyXmlMsgs.getAttribute(Slot.getParentNode(), XmlMsgs.prefix, "entity");
                     
                     
                     String real_owner_URI = owner_URI;
                     
                     // if the owner slot is child of a comparator
                     ...
                     {
                        ///////////////mesa sto GRE 8A PREPEI NA MPOYN...
                        String ret = GRE(owner_URI, Slot);
			...
			else
			{
                ...
                else if(XmlMsgs.compare(Slot, XmlMsgs.prefix, "LOCATION"))
                {
                        ...
                        //  if ((j+1)<MsgSlots.size())
                        {
                                ...
                                if (XmlMsgs.getAttribute(((Node)MsgSlots.get(j)).getParentNode(), XmlMsgs.prefix, "form").equalsIgnoreCase(""))
                                        singular = true;

                                String owner_URI = Slot.getTextContent(); 
                                String originalURI = owner_URI;
                                if(owner_URI.indexOf("-")>=0)
                                    owner_URI = owner_URI.substring(0,owner_URI.indexOf("-"));
                                ...
                                if (!owner_URI.equalsIgnoreCase(""))
                                {
                                        ...
                                        else
                                        {
                                            if(Languages.isGreek(getLanguage()))
                                            {
                                                ...
                                                {
                                                    int pos = NP.indexOf("βρίσκεται");
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"βρισκεται".length()) + " τώρα"+NP.substring(pos+"βρίσκεται".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("βρισκονται")+ "βρισκονται".length()) + " τώρα"+NP.substring(NP.indexOf("βρισκονται")+"βρίσκονται".length());
                                                }
                                                if(originalURI.indexOf("Also")>0)
                                                {
                                                    int pos = NP.indexOf("βρίσκεται");
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"βρισκεται".length()) + ", επίσης,"+NP.substring(pos+"βρίσκεται".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("βρισκονται")+ "βρισκονται".length()) + ", επίσης,"+NP.substring(NP.indexOf("βρισκονται")+"βρίσκονται".length());
                                                }
                                                if(originalURI.indexOf("Still")>0)
                                                {
                                                    int pos = NP.indexOf("βρίσκεται");
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"βρισκεται".length()) + ", ακόμα,"+NP.substring(pos+"βρίσκεται".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("βρισκονται")+ "βρισκονται".length()) + ", ακόμα,"+NP.substring(NP.indexOf("βρισκονται")+"βρίσκονται".length());
                                                }
                                            }
                                            else
                                            {
                                                ...
                                                if(originalURI.indexOf("Also")>0)
                                                {
                                                    ...
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"is".length()) + " also"+NP.substring(pos+"is".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("are")+ "are".length()) + " also"+NP.substring(NP.indexOf("are")+"are".length());
                                                }
                                                if(originalURI.indexOf("Now")>0)
                                                {
                                                    ...
                                                    int pos = NP.indexOf("is");
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"is".length()) + " now"+NP.substring(pos+"is".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("are")+ "are".length()) + " now"+NP.substring(NP.indexOf("are")+"are".length());
                                                }
                                                if(originalURI.indexOf("Still")>0)
                                                {
                                                    int pos = NP.indexOf("is");
                                                    if (pos>0)
                                                        NP = NP.substring(0,pos +"is".length()) + " still"+NP.substring(pos+"is".length());
                                                    else
                                                        NP = NP.substring(0,NP.indexOf("are")+ "are".length()) + " still"+NP.substring(NP.indexOf("are")+"are".length());
                                                }
                                            }
                ...
                else if(XmlMsgs.compare(Slot, XmlMsgs.prefix, XmlMsgs.AYTOS_TAG))
                {
                  ...
                  //      if (XmlMsgs.getAttribute(((Node)MsgSlots.get(j+1)),XmlMsgs.prefix,"form").equalsIgnoreCase(""))
                    ...
                    //    Lex_Entry_GR entry = (Lex_Entry_GR)NLGLQM.getEntry(next_URI, getLanguage());
                        String NP = "";
                        //an xreiazetai kai se alles ptwseis 8a prepei na kratw kai thn ptwsh toy Ypoleimenoy
                        //to idio isxyei kai gia ton ari8mo
                        ...
                        if (!next_URI.equalsIgnoreCase(""))
			{
                ...
                {//UNIQUE_TAG
                    ...
                    if ((j+1)<MsgSlots.size())
                    {
                        ...
                        //if (XmlMsgs.compare((Node)MsgSlots.get(j-1), XmlMsgs.prefix, XmlMsgs.IS_A_TAG))
                        //    removePreviousWords();
                ...
                {
                    String next_URI = ((Node)MsgSlots.get(j+1)).getTextContent();
                    if (next_URI.endsWith("previous"))
                        next_URI = ((Node)MsgSlots.get(j+2)).getTextContent();
                    String owner_URI = Slot.getTextContent();
                    boolean singular = false;
                        ...
                        {//English                         
                             ...
                             if(obj instanceof Lex_Entry_EN)
                             {
                                ...
                                String Case = XmlMsgs.getAttribute(Slot,XmlMsgs.prefix,XmlMsgs.CASE_TAG);
                                 ...
                                 {
                                     //<COMPARISONS>
                                     String NP = null; //<COMPARISONS>
                                     
                                     if (XmlMsgs.getAttribute(Slot,XmlMsgs.prefix,"form").equalsIgnoreCase(""))
                                        NP = entry.get(Case);
                                     else
                                        NP = entry.get(Case,XmlMsgs.PLURAL);
                                     ...
                                     else
                                     {
                                         ...
                                         if(pre_Slot!=null&&pre_Slot.getTextContent().equalsIgnoreCase("ακόμα")&&XmlMsgs.getAttribute(Slot,XmlMsgs.prefix,"form").equalsIgnoreCase(""))
                                         {
                                            if(entry!=null)
                                            {
                                                REALIZED_TEXT.delete(REALIZED_TEXT.indexOf("ακόμα"),REALIZED_TEXT.length());
                                                
                                                addText("another " + NP);
                                                
                                            }
                                         }//</COMPARISONS>
                                         else if(Slot.getPreviousSibling() != null 
                                         ...
                                         else
                                         {
                                             ...
                                             else
                                             {
                                                 ...
                                                 {
                                                    String ret = NP;
						    if (entry.get_num().equalsIgnoreCase("singular"))
							ret = EnglishArticles.getIndefiniteArticle(NP) + " " + NP;
                                                    setProduced_RE(Slot, SurfaceRealization.PROD_RE_NP); 
                                                    AnnotDescription.add_RE(ret, Filler_URI, re_type, get_for_Property(Slot), getProduced_RE(Slot), get_RE_Role(Slot), get_Interest(Slot), get_Assim(Slot));
                                                    addText(ret);
                                                 }
                ...
                else if(Languages.isGreek(getLanguage()))
                {
                     ...
                     else
                     {
                         ...
                         {  
                            ...
                            + " " + SurfaceRealizationHelper.getfoo2(LE_GR.get_Gender(), LE_GR.get_num(), OwnerCase) 
                            +  " " + LE_GR.get(OwnerCase);
                         }                                      
                         if(XmlMsgs.getAttribute(Slot, XmlMsgs.prefix, XmlMsgs.RE_FOCUS).compareTo(XmlMsgs.FOCUSLevel3)==0)
                         {
                            setProduced_RE(Slot, this.PROD_RE_NP);   
                            return  SurfaceRealizationHelper.getfoo(LE_GR.get_Gender(), LE_GR.get_num(), OwnerCase, true) 
                            +  " " + LE_GR.get(OwnerCase);
                         }
                         else if(XmlMsgs.getAttribute(Slot, XmlMsgs.prefix, XmlMsgs.RE_FOCUS).compareTo(XmlMsgs.FOCUSLevel2)==0)
                         { 
		...
		if(isAnon(nextFillerURI, FillerCase))
                ...
                else
                {
                    ...
                    else if(Languages.isEnglish(getLanguage()))
                    {
                         ...
                         {
                                setProduced_RE(Slot, this.PROD_RE_Demonstrative);   
                                //Lex_Entry_EN LE_EN = (Lex_Entry_EN)NLGLQM.getEntry(nextFillerURI, getLanguage(), this.UserType);
                                return "this"; // prepei na valw kai to these                           
                         }
                    }           
                }
            }
            else
            {
                ...
                //return GREforAnonEntity( ClassType, OwnerCase, Slot); //!!!!!!
    ...
    private String GREforNotAnonEntity(String entityURI,String OwnerCase,Node Slot,  boolean b)
    {
        ...
        // if owner is a class and not belongs to a comparator
        if(NLGEngine.isClass(ontModel, entityURI) && !XmlMsgs.compare(Slot.getParentNode(), XmlMsgs.prefix, XmlMsgs.COMPARATOR_TAG)) 
        {
            ...
            if(Languages.isEnglish(getLanguage()))
            {
                ...
                //EnglishArticles.getIndefiniteArticle(LE_EN.get(OwnerCase));
                if(LE_EN.getCountable())
                {
                    this.setProduced_RE(Slot, this.PROD_RE_NP);
		    if (LE_EN.get_num().equalsIgnoreCase("singular"))
			return EnglishArticles.getIndefiniteArticle(LE_EN.get(OwnerCase))+ " " + LE_EN.get(OwnerCase);
		    else
                        ...
                        if (exist("previous",Slot.getParentNode().getAttributes()))
                         {
                             ...
                             {
                                 this.setProduced_RE(Slot, this.PROD_RE_NP);
                                 return  LE_GR.get(OwnerCase, XmlMsgs.SINGULAR); 
                             }
                         }
                         else
                         {
    ...
    // return true if entityURI is mapped to a null
    // entity or an empty string (NP or canned text)
    ...
    // search next slots anf returns the uri of the nex filler         
    ...
    // search next slots anf returns the next filler Slot
    ...
    public void removePreviousWords() 
    {
        if (REALIZED_TEXT.indexOf(",")<0 && REALIZED_TEXT.indexOf(".")<0) 
        {
            if (getLanguage().equalsIgnoreCase("El")) 
            {
                String tmp = REALIZED_TEXT.substring(0, REALIZED_TEXT.indexOf("είναι"));
                REALIZED_TEXT.delete(0,REALIZED_TEXT.length()-1);
                
                REALIZED_TEXT.append(tmp+"είναι ");
            }
            else 
            {
    ...
    public String getReferringExprFromUser(Node Slot, String Gender, String number, String Case, String text)
    {
        if(XmlMsgs.getAttribute(Slot, XmlMsgs.prefix, XmlMsgs.RETYPE).compareTo(XmlMsgs.RE_AUTO)!=0)
        {
            ...
            else if (retype.compareTo(XmlMsgs.RE_DEF_ART)==0)
            {
                ...
                {           
                    if(Slot.getPreviousSibling()!=null && Slot.getPreviousSibling().getTextContent().compareTo("σε")==0)
                    {
                        setProduced_RE(Slot, this.PROD_RE_NP);
            ...
            else if (retype.compareTo(XmlMsgs.RE_INDEF_ART)==0)
            {
                ...
                {
                    setProduced_RE(Slot, this.PROD_RE_NP);
		    if (number.equalsIgnoreCase("singular"))
                        return EnglishArticles.getIndefiniteArticle(text) + " " + text;
		    else
			return text;
